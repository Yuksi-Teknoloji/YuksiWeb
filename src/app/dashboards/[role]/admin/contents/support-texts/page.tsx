...
/* -------- Modal (Referanslarda yaptığımızın aynısı) -------- */
...
function CreateModal({
  ...
  // kind bu sayfada sabit 'support', ama UI aynı gözüksün diye disabled select gösteriyoruz
  const kind = 'support';
    ...
    <div className="fixed inset-0 z-50 grid place-items-start overflow-y-auto bg-black/50 p-4">
      <div className="mx-auto w-full max-w-6xl rounded-2xl bg-white shadow-xl">
        ...
        <form onSubmit={submit} className="space-y-6 p-5">
          ...
          {/* İçerik Türü (sabit support) */}
          <div>
            <label className="mb-2 block text-sm font-medium text-neutral-700">İçerik Türü</label>
            <select
              value={kind}
              disabled
              className="w-full cursor-not-allowed rounded-xl border border-neutral-300 bg-neutral-100 px-3 py-2 text-neutral-800 outline-none"
            >
