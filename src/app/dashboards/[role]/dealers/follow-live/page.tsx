...
/* ---------------- API Shapes ---------------- */
type DealerRestaurant = { id: string; name: string };
type CourierLiveRow = {
  courier_id: string;
...
/* ---------------- Page ---------------- */
export default function DealerFollowLivePage() {
  ...
  const loadRestaurants = React.useCallback(async () => {
    ...
    try {
      ...
      const mapped: DealerRestaurant[] = arr
  ...
  const loadLive = React.useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/yuksi/dealer/restaurants/${restaurantId}/couriers/live-locations`,
